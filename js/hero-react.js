(function () {
  ...
  renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 2)); renderer.outputColorSpace = THREE.SRGBColorSpace; renderer.toneMapping = THREE.ACESFilmicToneMapping; renderer.toneMappingExposure = 1.1; const shell = document.createElement("div"); shell.className = "react-court-shell"; shell.appendChild(renderer.domElement); mount.appendChild(shell); mount.classList.add("has-3d");
  ...
  const ball = new THREE.Mesh(new THREE.SphereGeometry(1.18, 96, 64), new THREE.MeshStandardMaterial({ map: texture, roughness: .72, metalness: .02, bumpMap: texture, bumpScale: .025 })); scene.add(ball); mount.querySelector(".react-court-shell").classList.add("is-3d");
